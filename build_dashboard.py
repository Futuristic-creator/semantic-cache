"""Generates dashboard.html: a self-contained report embedding results.json."""
import json
...
const BACKEND_ORDER = ["naive", "gptcache", "redis", "adaptive"];
const COLORS = {
...
// shared legend under the two top charts
...
// category breakdown table at threshold = 0.25
...
// ---- live query panel (talks to app.py's Flask API, if running) ----
...
document.getElementById('liveRunBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    BACKEND_ORDER.forEach(key => {
      ...
      html += `<tr><td>${r.name}</td><td class="${r.hit ? 'ok-cell' : ''}">${r.hit ? 'HIT' : 'miss'}</td>` +
